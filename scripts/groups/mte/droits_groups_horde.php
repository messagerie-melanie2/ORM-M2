...
foreach ($groups_uid as $group_name => $group_uid) {
  ...
  // Filtres LDAP
  $findString = "mineqRDN=_agents_majauto_";
  $restriction_search = array( "memberUid" );
  $restriction_read = array( "mineqTypeEntree" );
      ...
      if (count($arrayValues) == 2) {
        ...
        if ($non_auto_racine) {
          ...
          if (strpos($row->attribute_key,$findString) !== FALSE) {
            ...
            if ($info['count'] > 0 && $info[0]['mineqtypeentree'] && is_array($info[0]['mineqtypeentree'])) {
              if ($cfg_trace) { echo "mineqTypeEntree: " . $info[0]['mineqtypeentree'][0] . "<br />"; }
              if ($info[0]['mineqtypeentree'][0] != 'NUNI') continue;
            }
          }
        }
