...
class CalendarSync extends Melanie2Object {
	...
	*/
	 ...
	 * @param integer $limit [Optionnel]
	 * @param string $startDate [Optionnel] Date de debut pour les événements retournés
	 */
	public function listCalendarSync($limit = null, $startDate = null) {
		...
		if (isset($this->token)) {
			$operators = ['token' => \LibMelanie\Config\MappingMelanie::sup];
			...
			if (isset($startDate)) {
				...
				$events = $event->getList(['uid'], null, ['start' => \LibMelanie\Config\MappingMelanie::sup]);
			}
			else {
