	...
	*/
	...
	public function __construct($eventParent = null, $usermelanie = null, $calendarmelanie = null) {
		...
		if (isset($eventParent)) {
			if (!isset($usermelanie)) $usermelanie = $eventParent->getUserMelanie();
			if (!isset($calendarmelanie)) $calendarmelanie = $eventParent->getCalendarMelanie();
		}
	...
	*/
	...
	function save() {
		...
		if (!is_null($insert)) {
			...
			if (is_null($insert)) $this->deleted = true;
