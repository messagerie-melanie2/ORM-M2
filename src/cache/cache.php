...
class Cache {
	...
	private static function memcache_connect() {
	    ...
	    if (strpos(ConfigMelanie::CACHE_MEMCACHE_HOST, ',') !== false) {
	        $memcache_hosts = explode(',',ConfigMelanie::CACHE_MEMCACHE_HOST);
	        foreach ($memcache_hosts as $memcache_host) {
	            if (isset($memcache_host)
	                    && $memcache_host != "") {
    	            $url = explode(':', $memcache_host);
    	            $host = $url[0];
    	            if (isset($url[1])) $port = $url[1];
    	            self::$memcache_cnx->addserver($host, intval($port));
	            }
	        }
	    } else {
    	    $url = explode(':', ConfigMelanie::CACHE_MEMCACHE_HOST);
    	    $host = $url[0];
    	    if (isset($url[1])) $port = $url[1];
	...
	static function setSQLToCache($table_name = null, $fields, $query, $params, $results) {
	    ...
	    if (!ConfigMelanie::CACHE_ENABLED) return false;
	    if (empty($table_name)) {
	        // On essaye de récupérer le nom de table depuis la query
	        if (strpos(strtoupper($query), "SELECT ") === 0
	                || strpos(strtoupper($query), "DELETE ") === 0) {
	            if (preg_match_all('/\b(from|join)\b\s*(\w+)/', strtolower($query), $matches)
	                    && isset($matches[2])) {
	            ...
	            if (preg_match('/\binto\b\s*(\w+)/i', strtolower($query), $matches)
	                    && isset($matches[2])) {
	            ...
	            if (preg_match('/\bupdate\b\s*(\w+)/i', strtolower($query), $matches)
	                    && isset($matches[1])) {
	    ...
	    // Cache mémoire php
	    if (ConfigMelanie::CACHE_TYPE == 'php') {
	    ...
	    else if (ConfigMelanie::CACHE_TYPE == 'memcache') {
	        // Cache memcache
	        ...
	        if (self::$memcache_cnx->get($hash) !== false) {
	            ...
	            if (is_array($table_name)) {
	                foreach($table_name as $table) {
	                    ...
	                    if ($table_name_hashs !== false) {
	                        ...
	                        if (!in_array($hash, $table_name_hashs)) {
	                            ...
	                            self::$memcache_cnx->replace(md5(strtolower($table)), serialize($table_name_hashs), false, ConfigMelanie::CACHE_DELAY);
	                        }
	                    }
	                    else {
	                        $table_name_hashs = [];
    	            ...
    	            if ($table_name_hashs !== false) {
    	                ...
    	                if (!in_array($hash, $table_name_hashs)) {
    	                    $table_name_hashs[] = $hash;
    	                    self::$memcache_cnx->replace(md5(strtolower($table_name)), serialize($table_name_hashs), false, ConfigMelanie::CACHE_DELAY);
    	                }
    	            }
    	            else {
    	                $table_name_hashs = [];
	        ...
	        self::$memcache_cnx->add($hash, serialize($results), false, ConfigMelanie::CACHE_DELAY);
	    }
	    return true;
	}
	...
	static function getFromSQLCache($table_name = null, $fields, $query, $params) {
	    ...
	    if (ConfigMelanie::CACHE_TYPE == 'php') {
    	    ...
    	    if (isset(self::$cache_sql[$table_name])
    	            && isset(self::$cache_sql[$table_name][$hash_fields])
    	            && isset(self::$cache_sql[$table_name][$hash_fields][$hash_query])
    	            && isset(self::$cache_sql[$table_name][$hash_fields][$hash_query][$hash_params])) {
    	        // Delai de rétention du cache
    	        if ((time() - self::$cache_sql[$table_name][$hash_fields][$hash_query][$hash_params][self::TIME]) > ConfigMelanie::CACHE_DELAY) {
    	            ...
    	            return null;
    	        } else return self::$cache_sql[$table_name][$hash_fields][$hash_query][$hash_params][self::RESULTS];
    	    }
	    }
	    else if (ConfigMelanie::CACHE_TYPE == 'memcache') {
	        // Cache memcache
