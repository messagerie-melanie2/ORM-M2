...
class Event extends Melanie2Object {
	...
	*/
	...
	function save() {
		...
		if (isset($this->deleted_exceptions)
		    && count($this->deleted_exceptions) > 0) {
	...
	protected function setMapExceptions($exceptions) {
	  ...
	  if (isset($this->exceptions)
	    ...
	    foreach($this->exceptions as $_exception) {
          ...
          if ($_recId == $recId
              && (!$exception->deleted || $_exception->deleted)) {
            $deleteEx = false;
            break;
          }
	      }
	...
	protected function getMapExceptions() {
		...
		if (!isset($this->objectmelanie)) throw new Exceptions\ObjectMelanieUndefinedException();
		if (!isset($this->objectmelanie->exceptions) || $this->objectmelanie->exceptions == "") return [];
